import primary.Elevator;
import primary.Floor;
...
public class ECSTest {
    ...
    public void testReadFromFileMultipleFloors() throws InterruptedException{
        final String FILE_PATH = "src/test/resources/inputReadFromFileMultipleFloors.txt";
        ...
        for (int i = 0; i < MAX_FLOORS; i++){
            ...
            Thread floorSubsystem = new Thread(floor, "Floor " + i);
            floors.add(floor);
            floorThreads.add(floorSubsystem);
        }
        Thread scheduler = new Thread(new Scheduler());
        Thread elevator = new Thread(new Elevator(1));
        scheduler.start();
        for (Thread t : floorThreads) t.start();
        elevator.start();
